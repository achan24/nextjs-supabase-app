...
const Timeline = ({ timelineEvents, onAddEvent, onRemoveEvent, onRepositionEvent }) => {
  const [currentDate, setCurrentDate] = useState(new Date())
  const [zoomLevel, setZoomLevel] = useState('day') // 'hour', 'day', 'month', 'year'
    ...
    switch (zoomLevel) {
      ...
      case 'day':
        return {
          ...
          getUnitDate: (index) => addDays(currentDate, index - 3), // Center current date (3 days before, 3 after)
          formatLabel: (date) => {
      ...
      case 'month':
        return {
          ...
          unitWidth: 150,
          getUnitDate: (index) => addMonths(currentDate, index - 6),
      ...
      case 'year':
        return {
          ...
          unitWidth: 200,
          getUnitDate: (index) => addYears(currentDate, index - 5),
          formatLabel: (date) => format(date, 'yyyy'),
          unitLabel: 'Year'
        }
      default:
        return getTimelineConfig('day')
    }
  }
  ...
  const getTimeAtPosition = (clientX) => {
  ...
  const [{ isOver }, drop] = useDrop({
    ...
    hover: (item, monitor) => {
      ...
      if (clientOffset) {
        ...
        setHoverTime(time)
        setMousePosition({ x: clientOffset.x, y: clientOffset.y, relativeX: clientOffset.x - (timelineRef.current?.getBoundingClientRect().left || 0) })
      }
    },
    drop: (item, monitor) => {
