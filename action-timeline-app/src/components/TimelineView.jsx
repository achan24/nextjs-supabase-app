...
const TimelineView = ({ timelineEngine }) => {
  ...
  const renderTimelineNode = (node, index, isActive = false, isCompleted = false) => {
    const getNodeIcon = () => {
      if (node.type === 'action') {
          ...
          <div className="flex-1 min-w-0">
            ...
            {node.type === 'decision' && node.status === 'completed' && node.selectedOption && (
              <div className="mt-2 text-sm text-green-600">
                Selected: {node.options.find(opt => opt.actionId === node.selectedOption)?.label || 'Option'}
              </div>
            )}
    ...
    if (currentId) {
      ...
      if (currentNode) {
        ...
        if (currentNode.type === 'action') {
          ...
          nextNodes = currentNode.options.map(opt => timelineEngine.getNode(opt.actionId)).filter(Boolean);
        }
