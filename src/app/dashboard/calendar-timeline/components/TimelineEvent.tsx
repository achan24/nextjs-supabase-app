...
interface TimelineEventProps {
  ...
  position: number;
  onRemove: () => void;
  onReposition: (eventId: string, newDate: Date) => void;
  zoomLevel: 'hour' | 'day' | 'month' | 'year';
  config: TimelineConfig;
}

const TimelineEvent = ({ event, position, onRemove, onReposition, zoomLevel = 'day', config }: TimelineEventProps) => {
  const [isHovered, setIsHovered] = useState(false);
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
  
  const [{ isDragging }, drag] = useDrag<TimelineEventData & { isRepositioning: boolean }, void, { isDragging: boolean }>({
    type: 'timeline-event',
